function afficherData() {
  ...
  fetch('data.json')
    ...
    .then(data => {
      ...
      afficherEnfantetPetitenfant(enfants, petitsEnfants, data);
    })
...
function afficherGrandsParents(personnes, personId, data) {
  ...
  if (id_mere) {
    ...
    if (id_gr_pere_mere) grandsParentsIds.add(id_gr_pere_mere);
    if (id_gr_mere_mere) grandsParentsIds.add(id_gr_mere_mere);
  }
  if (id_pere) {
    const [id_gr_pere_pere, id_gr_mere_pere] = [trouverIdParent(id_pere, data, 'pere'), trouverIdParent(id_pere, data, 'mere')];
    if (id_gr_pere_pere) grandsParentsIds.add(id_gr_pere_pere);
    if (id_gr_mere_pere) grandsParentsIds.add(id_gr_mere_pere);
  }
